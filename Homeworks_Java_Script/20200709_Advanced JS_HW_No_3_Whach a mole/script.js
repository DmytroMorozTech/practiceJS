class Menu {
    ...
    static _chooseDifficultyHTML;
    static _chooseFieldSize;
    ...
    _createMenu() {
        ...
        `;
        this._gameParams.insertAdjacentHTML("beforeend",Menu._chooseDifficultyHTML);
        this._gameParams.insertAdjacentHTML("beforeend",Menu._chooseFieldSize);
    ...
    _getLevelOfDifficulty() {
        ...
        return Menu._levels[chosenDifficultyLvl].value;
    }

    _getGameFieldSize() {
        let chosenFieldSize = document.querySelector('input[name="fieldSize"]:checked').value;
        return Menu._fieldSize[chosenFieldSize].value;
    }
    ...
    _onClickStart() {
        let rowsQuantity = this._getGameFieldSize();
        let columnsQuantity = this._getGameFieldSize();
        let timerForCellChange = this._getLevelOfDifficulty();

        const game = new GameField(rowsQuantity, columnsQuantity,timerForCellChange);
        this.game = game;
...
class GameField {
    constructor(rows=10,columns=10, timerForCellChange = 1000) {
        this._restartGameButton = document.querySelector(".restart-game-btn");
        this.timerForCellChange = timerForCellChange;
    ...
    _fillDataField() {
        this.dataField = Array.from({length: this._rowsQuantity},
            (item, rowIndex) => Array.from(
                {length:this._columnsQuantity},
                    (item, columnIndex) => new TableCell((rowIndex + 1), (columnIndex + 1))));
    }
    ...
    nextStep() {
        if(!this.winner) {
            ...
            // timerForCellChange - переменная, в которой хранится вышеуказанное значение.
        }
    }
