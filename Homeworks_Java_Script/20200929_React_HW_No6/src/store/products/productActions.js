...
export const loadProductsFromServer = () => (dispatch) => {
  ...
  setTimeout(() => {
    axios
      .get("./listOfProducts.json")
      .then((result) => result.data)
      .then((dataFromServer) => {
        const favoritesIDs = JSON.parse(
          localStorage.getItem("prodsInFavoritesStorage")
        );
        ...
        listOfProducts.forEach((product) => {
          if (favoritesIDs && favoritesIDs.includes(product.id)) {
            product.addedToFavorites = true;
          }
          if (shoppingCartIDs && shoppingCartIDs.includes(product.id)) {
