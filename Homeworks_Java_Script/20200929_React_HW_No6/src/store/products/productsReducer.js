  ...
  switch (action.type) {
    ...
    case actions.TOGGLE_CART:
      const shoppingCartIDs =
        JSON.parse(localStorage.getItem("prodsInCartStorage")) || [];
      const productWasInCart = shoppingCartIDs.includes(action.payload);

      if (!productWasInCart) {
        localStorage.setItem(
          "prodsInCartStorage",
          JSON.stringify([...shoppingCartIDs, action.payload])
        );
      } else {
        const updatedShoppingCardId = [...shoppingCartIDs].filter(
          (id) => id !== action.payload
        );
        localStorage.setItem(
          "prodsInCartStorage",
          JSON.stringify(updatedShoppingCardId)
        );
      }
      // ------------------------
      ...
      const updatedProductInCart = {
        ...productInCart,
        addedToCart: !productInCart.addedToCart,
      };
      return update(store, {
        products: { $splice: [[productInCartIndex, 1, updatedProductInCart]] },
      });
    // --------------------------------------
    case actions.TOGGLE_FAVORITES:
      ...
      const favoritesIDs =
        JSON.parse(localStorage.getItem("prodsInFavoritesStorage")) || [];
      const productWasInFavorites = favoritesIDs.includes(action.payload);

      if (!productWasInFavorites) {
        localStorage.setItem(
          "prodsInFavoritesStorage",
          JSON.stringify([...favoritesIDs, action.payload])
        );
      } else {
